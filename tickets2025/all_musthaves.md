...
* Конкатенация: [Пример](https://github.com/hse-spb-2021-cpp/lectures/blob/master/08-211020/01-c-str/05-allocator.cpp) — это операция соединения нескольких строк символов в одну. В информатике — это объединение двух или более объектов счётного вида с сохранением порядка следования элементов (списков, строк, массивов, кортежей).
...
* Move в C++: ![image](https://github.com/user-attachments/assets/6d57db1e-82a7-4cbf-a044-4c5b882ac2a8)
* lvalue в C++ — это выражение, которое имеет идентичность, но не может быть неявным образом перемещено. Фактически это всё, чему может быть присвоено значение, например, переменная, результат разыменовывания указателя, ссылка. 
xvalue (от expiring value) — это выражение, которое имеет идентификатор и представляет объект, из которого можно неявно перемещать. Общая идея в том, что объект, который они представляют, скоро будет уничтожен, и от него можно неявно переместиться. 
Примеры выражений lvalue: имена переменных, включая const переменные, элементы массива, вызовы функций, возвращающие ссылку lvalue, бит-поля, объединения и члены класса. Примеры выражений xvalue: вызовы функций, возвращающие ссылку rvalue, а также подстрочный массив, член и указатель на выражения элементов, где массив или объект является ссылкой rvalue. 
...
* Копировать `unique_prt` нельзя, потому что он `unique`, очев же (это сделано, чтобы случайно не
  ...
  можно забрать значение (передать владение), тогда `foo` занулится. В результате исходный unique_ptr становится пустым (то есть становится nullptr, а ресурс (память) переходит к новому unique_ptr).
# Полиморфные классы
...
* Шаблон класса (class template) позволяет задать внутри класса объекты, тип которых на этапе написания кода неизвестен. Для определения шаблона класса применяется следующий синтаксис:
```c++
...
* В современном, т.е. C++17 варианте языка разницы между class и typename применительно к шаблонам нет. Однако, в более ранних стандартах разница была заметна, когда в качестве аргумента шаблона выступает шаблонный же тип. В этом случае использование typename перед именем шаблонного типа не являлось допустимым.
